<template>
  <div class="older-page_bg">
    <page-header title="全区居家养老服务情况统计分析"></page-header>
    <div class="older-container">
      <div class="bread">
        <norm-layout
          text="五大服务类别数据综合分析——全区居家养老服务情况统计分析"
          path="FiveService"
          :flag="false"
        ></norm-layout>
      </div>
      <div class="from_header">
        <el-form>
          <el-form-item label="服务时间：" class="search_select first_item">
            <el-date-picker
        ...
        <p class="service_content">
          <span
            >服务量：{{ serviceAllTotalMap.orderAllCardMonthTotal }}客次</span
          >
          <span>消费金额：{{ serviceAllTotalMap.consumerAllMoney }}元</span>
        </p>
      </div>
      <div class="main">
...
export default {
  ...
  data() {
    return {
      ServiceTime: [new Date(new Date() - 1000 * 60 * 60 * 24 * 7), new Date()],
  ...
  methods: {
    ...
    getmerchantServicesList() {
      ...
      let params =
        roleid === "28" ? this.params : { companyName: this.form.companyName };
      api.getmerchantServicesList(params).then((res) => {
        if (res.data) {
          this.merchantList = res.data.merchantMap.map(
            (item) => item.MerchantName
          );
          this.form.merchantName =
            this.merchantList.length <= 1 ? this.merchantList[0] : "";
        }
      });
    },
    ...
    getServerOrderInfo() {
      ...
      this.form.beginDay = this.ServiceTime[0];
      this.form.endDay = this.ServiceTime[1];
      this.form.serverFlag = this.form.beginDay || this.form.endDay ? "" : "1";
      let form = Object.assign({}, this.form, this.params);
      api.getServerOrderInfo(form).then((res) => {
        let { serviceAllTotalMap, serviceTotalMap } = res.data;
        if (res.data) {
          this.isSearchLoading = false;
          this.serviceAllTotalMap = serviceAllTotalMap;
          if (serviceTotalMap) {
            this.serviceData.xData = serviceTotalMap.map((item) => {
              if (item) {
                switch (item.weekDay) {
                  case 1:
                    item.weekText = "周日";
                ...
                return item.mealTime.slice(5, 10) + "\n" + item.weekText;
              } else {
                return [];
              }
            });
            this.serviceData.zValue = serviceTotalMap
              ? serviceTotalMap.map((item) => item.orderCardMonthTotal)
              : [];
            this.serviceData.rValue = serviceTotalMap
              ? serviceTotalMap.map((item) => item.consumerMoney)
              : [];
          } else {
            this.serviceData = {
              xData: [],
...
.from_header {
  ...
  // 防止起止日期折行
  /deep/ .first_item .el-form-item__content {
    display: flex;
