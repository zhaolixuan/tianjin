    ...
    <div class="worker_container">
      ...
      <dl v-for="item in ServiceData" :key="item.title" :class="lineWidth">
        <dt v-if="item.icon">
          <img
            :src="
              require(`@/views/OlderDistribution/assets/icons/${item.icon}.png`)
            "
            class="icon"
            alt="norm-icon"
          />
        ...
        <dd v-if="flag == 'BuiltService'">
          ...
          <p class="service_val" v-if="item.number">
        ...
        <dd v-else>
          ...
          <div @click="handleRouterLink(item.path)">
            <p class="service_item">{{ item.title || "" }}</p>
            <p class="service_val" v-if="item.number">
              <span v-if="!item.animation">{{ item.number || 0 }}</span>
              <countTo
...
export default {
  ...
  methods: {
    handleRouterLink(path) {
      if (path != undefined) {
        this.$router.push({ path: path.name ,query:path.query});
      }
       // console.log(path);
    },
  },
