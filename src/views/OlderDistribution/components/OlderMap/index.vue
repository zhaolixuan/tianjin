            ...
            this.scatterSingleStreetData = filteredGeoPoints.map(function(
              ...
              if (values.length > 0) {
                ...
                let importantOldManValue = {
                  typeUp80: parseInt(values[0].ninetyTotal), // 80岁以上
                  typeAloneTotal: parseInt(values[0].isAloneTotal), // 独居
                  typeIsLiveAlonelTotal: parseInt(values[0].isLiveAlonelTotal), // 失独
                  typeDisabilityTotal: parseInt(values[0].disabilityTotal), // 失能
                  typePensionTotal: parseInt(values[0].pensionTotal), // 低收入
                  typeDisabledTotal: parseInt(values[0].isDisabledTotal) // 残疾人
                };
                value.push(
                  feature.longitude,
                  feature.latitude,
