<template>
  <div class="breadcrumb-container">
    <span v-for="(item, index) in breadCrumbs"
      ...
      {{ item.name }}
      <span v-if="breadCrumbs.length > 0 && index + 1 !== breadCrumbs.length">/</span>
    </span>
  </div>
</template>
...
export default {
  ...
  data () {
    return {
      regions: [
        { name: '河西区', code: '120103000000', parentCode: null, type: 1 },
        {
          name: '东部片区',
          code: '120001',
  ...
  computed: {
    ...
    breadCrumbs () {
      const hexiRegion = this.communities.find(item => item.name === '河西区')
      ...
      return hexiRegion ? buildFullTree(hexiRegion, this.code[0]) : []
    }
  },
  watch: {
    type (val) {
  ...
  methods: {
    ...
    handleSetType (type, region) {
      ...
      this.$store.commit('setCode', [region.code, parentCode])
    }
  },
  created () {
