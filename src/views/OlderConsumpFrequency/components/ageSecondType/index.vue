...
import {defaultOptions, createBar, createYaxis} from '../ServiceType/option.js'
...
export default {
  ...
  computed: {
    computedOptions () {
      ...
      let { labelWidth, data, label, title } = this.MedircalData
      return Object.assign({}, option, {
        yAxis: createYaxis(title),
        series: createBar(data, labelWidth, label)
      })
    }
  },
  methods: {
...
.chart_help{
  // 此处设置为 2.5rem 高度故意让其比 ageSecond/index.vue(此文件的父组件)
  // 高的 因为这样既能保持 echarts 图宽度和其它组件一致 又不会撑开此组件的
  // 祖父组件 OlderConsumpFrequency/index.vue 的布局 
  height: 2.5rem;
  padding-left: .2rem;
}
</style>
