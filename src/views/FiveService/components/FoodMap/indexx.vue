...
<template>
  <div style=" height: 100%;width: 100%;">
    ...
    <CheckLegend ref="myCheckLegend"
                 v-show="isShowLegend"
                 @checkShowHexiQu='hideShowLegendHexiQu'
                 @addPieChart='addPieChart'></CheckLegend>
    <div ref="myMapChart"
...
import {
  getbyZoneCode, getByParentId, getBasePersonInfoNumber, getBasePersonInfoNumberEmphasis
} from '@/views/OlderDistribution/apis/index.js'
import { getCaterServiceToday } from '@/views/BuffetPage/apis/index.js'
import * as turf from '@turf/turf'
import streetData from './streetData.js'
// import ChangeMapTab from '@/views/OlderDistribution/components/OlderMap/ChangeMapTab/index'
...
export default {
  ...
  methods: {
    ...
    loadChart (bussinesType) {
      ...
      if (this.typeNew == 1) { // 1：河西区  2：街道 3：社区
        this.getAllStreetMap('', '120103000000', bussinesType)
      } else if (this.typeNew == 3) {
        this.getSingleStreetMap(myCode, parentCode, bussinesType)
    ...
    clickEvent () {
      ...
      _self.myChart.on('click', function (params) {
        ...
        try {
          ...
          switch (level) {
            case 3:
              _self.getSingleStreetMap(myCode, myparentCode, _self.bussinesType)
              _self.$store.commit('setCodeNew', [params.data.code, params.data.parent_code])
    ...
    getStreetPoints (code, parentCode, registerMapName) {
      ...
      getbyZoneCode().then(res => {
        ...
        if (code === '') {
          filteredGeoPoint = streetPointsJson.data.filter(item => item.code !== code)
        } else {
          filteredGeoPoint = streetPointsJson.data.filter(item => item.code === code)
        }
        ...
        if (code === '') {
          for (var i = 0; i < streetData.data.length; i++) {
            var newFilter = filteredGeoPoint.filter(item => item.name === streetData.data[i].name)
            if (newFilter.length > 0) {
              countryData.push(_self.setAreaColor(streetData.data[i].name, streetData.data[i].value,
                streetData.data[i].color, newFilter[0].code, newFilter[0].parentCode, 3))
            }
          }
        } else {
          // 查询某个街道行政边界
          var singleStreetData = streetData.data.filter(item => item.code === code)
          var newFilter2 = filteredGeoPoint.filter(item => item.code === code)
          if (newFilter2.length > 0) {
            countryData.push(_self.setAreaColor(singleStreetData[0].name, singleStreetData[0].value,
              singleStreetData[0].color, newFilter2[0].code, newFilter2[0].parentCode, 3))
          }
        }
     ...
     */
    getImportantOldMan (filteredGeoPoints, dataList, type, _color, nameType) {
      var data = []
      filteredGeoPoints.map(function (feature) {
        if (feature.properties.type === type) {
          for (var indexCode in dataList) {
            // 遍历json数组时，这么写p为索引，0,1
            if (indexCode === feature.properties.code) {
              var item = dataList[indexCode]
              let value = item[0][nameType] // parseInt(Math.random() * 10000)
              let newCoor = [feature.geometry.coordinates[0], feature.geometry.coordinates[1]]
              data.push({
 ...
 */
    getOldManPoints (code, parentCode, registerMapName) {
      ...
      let filteredGeoPoint
      if (code === '') {
        filteredGeoPoint = oldManPosition.features.filter(feature => feature.properties.code !== code)
      } else {
        filteredGeoPoint = oldManPosition.features.filter(feature => feature.properties.code === code)
      }
      getCaterServiceToday({
        identify: '0',
        code: '',
        parentcode: ''
      }).then(res => {
        var list = res.data.suborderMap
        var scatterDataUp80 = this.getImportantOldMan(filteredGeoPoint, list, 1, '#FDC400', 'peopleTotal')// 80岁以上
        // var scatterDataAloneTotal = this.getImportantOldMan(filteredGeoPoint, list, 2, '#4b0082')// 独居
        // var scatterDataLiveAlonelTotal = this.getImportantOldMan(filteredGeoPoint, list, 3)// 失独
        var scatterDataDisabledTotal = this.getImportantOldMan(filteredGeoPoint, list, 4, '#00FA9A', 'serviceOrderTotalFive')// 失能
        // var scatterDataPensionTotal = this.getImportantOldMan(filteredGeoPoint, list, 5)// 低收入
        // var scatterPhysicallyDisabledTotal = this.getImportantOldMan(filteredGeoPoint, list, 6)// 残疾人

        var countryData = []

        // 查询全部街道行政边界
        if (code === '') {
          var allStreetData = streetData.data.filter(item => item.code !== code)
          for (var i = 0; i < allStreetData.length; i++) {
            countryData.push(_self.setAreaColor(allStreetData[i].name, allStreetData[i].value,
              allStreetData[i].color, allStreetData[i].code, allStreetData[i].parentCode, 3))
          }
        } else {
          // 查询某个街道行政边界
          var singleStreetData = streetData.data.filter(item => item.code === code)
          ...
          countryData.push(_self.setAreaColor(singleStreetData[0].name, singleStreetData[0].value,
            singleStreetData[0].color, singleStreetData[0].code, singleStreetData[0].parentCode, 3))
        }
        let option = {
        ...
        var seriesDisabledTotal = this.getImportantOldManSeries(scatterDataDisabledTotal)
        // var seriesPensionTotal = this.getImportantOldManSeries(scatterDataPensionTotal)
...
*/
    getAllStreetMap (code, parentCode, bussinesType) {
      ...
      if (code === '') {
        ...
        filterGeoPolygon = hexigeojson.features.filter(item => item.properties.code === code)
      }
...
* bussinesType 业务类型 老年人分布为1 ；重点关注老年人分布为2
*/
    getSingleStreetMap (code, parentCode, bussinesType) {
      var _self = this
      var myJson = hexiPianquStreet
      // 显示某个街道底图层
      let geojson = JSON.parse(JSON.stringify(myJson))
      const filterGeo = geojson.features.filter(item => item.properties.code === code)
      let registerMapName = 'tianjin_hexi_streetmap' + (new Date()).valueOf()
      ...
      getByParentId({
        ...
        countryData.push(_self.setAreaColor(filterGeo[0].properties.name, 30000, _areaColor, filterGeo[0].properties.code, filterGeo[0].properties.parentCode, 20))
        let option = {
          tooltip: {
            trigger: 'item',
            backgroundColor: '#09bdb1',
            ...
            formatter: function (params, ticket, callback) {
              ...
              if (bussinesType === 2) {
                if (params.seriesType === 'effectScatter') {
                  ...
                  // var value = params.value
                  res = '<span style=\'color:#fff;\'>' + kind + '</span>' + value2 + '人'
                }
                // }
              }
        ...
        const filteredGeoPoints = shequPointsJson.filter(item => item.parentCode === code)
        // 老年人分布情况
        getCaterServiceToday({
          identify: '1',
          streetCode: code,
          ...
          this.scatterSingleStreetData = filteredGeoPoints.map(function (feature) {
            ...
            if (values) {
              ...
              let importantOldManValue = {
                typeUp80: parseInt(values.peopleTotal), // 80岁以上
                typeAloneTotal: parseInt(values.serviceOrderTotalFive) // 独居

              }
              value.push(feature.longitude, feature.latitude, importantOldManValue)
              return {
                code: feature.code,
  ...
  */
    getSingleShequMap (code, parentCode, bussinesType) {
      ...
      getByParentId({
        ...
        // 需要修改为真实值
        var _areaColor = streetData.data.filter(item => item.name === filterGeo[0].properties.name)[0].color
        countryData.push(_self.setAreaColor(filterGeo[0].properties.name, 30000, _areaColor, filterGeo[0].properties.code,
          filterGeo[0].properties.parentCode, 40))
        let option = {
          tooltip: {
            trigger: 'item',
            backgroundColor: '#09bdb1',
        ...
        getCaterServiceToday({
          ...
          // 重点老年人分布情况
          let suborderMap = res.data.suborderMap
          this.scatterSingleStreetData = filteredGeoPoints.map(function (feature) {
            var _code = feature.code
            var values = []
            for (var indexCode in suborderMap) {
