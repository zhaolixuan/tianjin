...
<template>
  <div style="height: 100%; width: 100%">
    <check-legend v-show="isShowLegend" @checkShow="checkShow"></check-legend>
    <detial-tip
      v-if="isShowDetail"
      :ztCode="myZtCode"
...
import { getCaterServiceTodayTwo } from "@/views/BuffetPage/apis/index.js";
...
import CheckLegend from "./CheckLegends.vue";
import DetialTip from "@/views/HomeDistribution/components/FoodMap/DetialTip/indes";
import iconJJ from "@/views/OlderDistribution/assets/img/jj.png";
import iconSQJ from "@/views/OlderDistribution/assets/img/sqj.png";
import iconFWD from "@/views/OlderDistribution/assets/img/fwd.png";
...
export default {
  ...
  methods: {
          ...
          switch (level) {
            ...
            case 10:
              ...
              _self.myDetailObject = {
                ...
                serviceArea: params.data.serviceArea,
                constructionScale: params.data.constructionScale,
                servicePeople: params.data.servicePeople,
                serviceRadiationRange: params.data.serviceRadiationRange,
                videoUrl: params.data.videoUrl
                  ? params.data.videoUrl
                      .split(",")
                      .map((item, index) =>
                        index !== 0 ? item.replace(/\n/g, "<br/>") : item
                      )
                  : [],
              };
              break;
     ...
     * 查询河西区所有街道中心点，及计算数据
     */
    async getStreetPoints(code, registerMapName) {
      var _self = this;
      await getbyZoneCode().then((res) => {
        // 获取所有街道中心点
    ...
    checkShow(val) {
      ...
      if (option && val) {
        option.series.map((item, id) => {
          val.map((ite) => {
            if (item.name === ite.text && !ite.show) {
              item.symbolSize = 0;
            } else if (item.name === ite.text && ite.show) {
              item.symbolSize = 40;
            }
          });
        });
        ...
        if (feature.properties.type === type) {
          for (var indexCode in suborderMap) {
            if (indexCode === feature.properties.code) {
              var item = suborderMap[indexCode];
              let value = item[type].canteenCount; // parseInt(Math.random() * 10000)
    ...
    getSingleStreetMap(code, parentCode) {
      ...
      getByParentId({
        ...
        getCaterServiceTodayTwo({
          ...
          var ztData = res.data.serviceAllOrderMap;
    ...
    getSingleStreetFoodServicesPoints(ztData, code, option, typeLevel) {
      ...
      var _iconSQJ = "image://" + iconSQJ;
      var _iconFWD = "image://" + iconFWD;
      var iconList = [_iconJJ, _iconSQJ, _iconFWD];
      var ztList = ["街级", "社区级", "服务站"];
      var singelFwPointDatas = []; // 服务载体集合
      for (var indexCode in ztData) {
        if (ztData[indexCode].length > 0) {
          ztData[indexCode].map((ite) => {
            var singelFwPointDataItem = {};
            singelFwPointDataItem.name = ite.MerchantName;
            singelFwPointDataItem.longitude = parseFloat(ite.longitude);
            singelFwPointDataItem.latitude = parseFloat(ite.latitude);
            singelFwPointDataItem.code = ite.community_Code;
            singelFwPointDataItem.parentCode = ite.street_code;
            singelFwPointDataItem.canteenlevel = ite.canteenlevel;
            singelFwPointDataItem.ztCode = ite.canteenNumber; // 牌匾号
            singelFwPointDataItem.constructionScale = ite.constructionScale; // 建设规模
            singelFwPointDataItem.servicePeople = ite.servicePeople; // 服务人员情况
            singelFwPointDataItem.serviceRadiationRange =
              ite.serviceRadiationRange; // 服务辐射范围
            singelFwPointDataItem.companyname = ite.companyname; // 服务商
            singelFwPointDataItem.enterprisename = ite.enterprisename; // 供餐企业
            singelFwPointDataItem.community = ite.community; // 服务区域
            singelFwPointDataItem.principal = ite.Principal; // 主要负责人
            singelFwPointDataItem.address = ite.address;
            singelFwPointDataItem.videoUrl = ite.videoaddress
              ? ite.videoaddress
              : "";
            singelFwPointDataItem.serviceArea = ite.serviceArea;
            singelFwPointDataItem.principalTelno = ite.principalTelno; // 联系电话
      ...
      for (let index = 0; index < ztList.length; index++) {
        ...
        var fwztData = filteredGeoPoint
          .filter((feature) => feature.canteenlevel === ztList[index])
          .map(function (feature) {
            ...
            return {
              ...
              constructionScale: feature.constructionScale,
              servicePeople: feature.servicePeople,
              serviceRadiationRange: feature.serviceRadiationRange,
