...
export default {
  data() {
    return {
      beginAndEndTime: [new Date(new Date("2021,6,1")), new Date()],
  ...
  created() {
    this.form.beginDay = this.beginAndEndTime
      ? formatTime(this.beginAndEndTime[0])
      : "";
    this.form.endDay = this.beginAndEndTime
      ? formatTime(this.beginAndEndTime[1])
      : "";
    if (
  ...
  methods: {
    ...
    getConsumeDistributionList() {
      ...
      let params = {
        beginDay:
          this.$route.query.beginDay ||
          this.beginAndEndTime[0] ||
          this.form.beginDay,
        endDay:
          this.$route.query.endDay ||
          this.beginAndEndTime[1] ||
          this.form.endDay,
        MinNum: this.$route.query.MinNum || this.streetDistriForm.consumeCount,
        MaxNum: this.$route.query.MaxNum || this.streetDistriForm.maxNum,
        ...
        classifyType: this.$route.query.classifyType || "street"
      };
      api.getConsumeDistributionList(params).then(res => {
        // console.log(res);
        this.isSearchLoading = false;
        var resData = res.data.reverse();
        this.serviceData.Data = returnData(resData, "street", "");
